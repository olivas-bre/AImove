...
class AeGom:
    ...
    def do_gom(self, eulerAngles):
        ...
        outputs = self.modelAE.predict(self.dataX)
        d1_pred = outputs[0]
        coef = outputs[1]
        
        d1_pred = keras.backend.get_value(d1_pred)
        self.coef_ae = keras.backend.get_value(coef)
        ...
        rc = np.concatenate((self.coef_ae[0,:,2,:],self.coef_ae[0,:,1,:],self.coef_ae[0,:,0,:]), axis=1)
        rc2 = rc.flatten()
        coef_mat_ae = pd.DataFrame(rc2[np.newaxis,:], columns=self.varCoef)
        
        for i in np.arange(1,len(self.coef_ae)):
            rc = np.concatenate((self.coef_ae[i,:,2,:],self.coef_ae[i,:,1,:],self.coef_ae[i,:,0,:]), axis=1)
            rc2 = rc.flatten()
