...
class VaeGom:
    ...
    def do_gom(self, eulerAngles):
        ...
        z_mean, z_log_var, z, encoder_last_h, encoder_last_c = self.vae_encoder(self.dataX)
        d1_pred, coef = self.vae_decoder([z, self.dataX, encoder_last_h, encoder_last_c])
        
        d1_pred = keras.backend.get_value(d1_pred)
        self.coef_vae = keras.backend.get_value(coef)
        ...
        rc = np.concatenate((self.coef_vae[0,:,2,:],self.coef_vae[0,:,1,:],self.coef_vae[0,:,0,:]), axis=1)
        rc2 = rc.flatten()
        coef_mat_vae = pd.DataFrame(rc2[np.newaxis,:], columns=self.varCoef)
        
        for i in np.arange(1,len(self.coef_vae)):
            rc = np.concatenate((self.coef_vae[i,:,2,:],self.coef_vae[i,:,1,:],self.coef_vae[i,:,0,:]), axis=1)
            rc2 = rc.flatten()
